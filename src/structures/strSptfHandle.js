/////////////////////// IMPORTS //////////////////////////
import { handleVideo } from './strSpotifyTrack.js';
import { handleVideo as _handleVideo } from './strSpotifyAlbum.js';
import { handleVideo as __handleVideo } from './strSpotifyPlaylist.js';
import SpotifyWebApi from 'spotify-web-api-node';
import { acess as _acess } from '../utils/acess.json';
...
spotifyApi.setAccessToken(_acess);
spotifyApi.setRefreshToken(process.env.SPOTIFY_KEY_REFRESH);
/////////////////////// SOURCE CODE ///////////////////////////
export async function handleSpotifyMusic(client, searchString, cath, message, voiceChannel) {
    ...
    if (searchString.includes("open.spotify.com/playlist")) {
        spotifyApi.getPlaylist(cath[2])
            .then(async function (data) {
                ...
                for (const track of tracks) {
                    await __handleVideo(client, track, message, voiceChannel, true);
                }
                ...
                if (err.message.includes("The access token expired.")) {
                    spotifyApi.refreshAccessToken().then(
                        async function (data3) {
                            ...
                            writeFile("./utils/acess.json", JSON.stringify(data3.body['access_token']), (err) => {
                                if (err) {
                                    console.error(err);
                                    return;
                                };
        ...
        spotifyApi.getTrack(`${cath[2]}`)
            .then(async function (data4) {
                ...
                await handleVideo(client, track, message, voiceChannel);
                return;
            }, function (err) {
                if (err.message.includes("The access token expired.")) {
                    spotifyApi.refreshAccessToken().then(
                        async function (data6) {
                            ...
                            writeFile("./utils/acess.json", JSON.stringify(data6.body['access_token']), (err) => {
                                if (err) {
                                    console.error(err);
                                    return;
                                };
        ...
        spotifyApi.getAlbumTracks(`${cath[2]}`)
            .then(async function (data5) {
                ...
                for (const track of tracks2) {
                    await _handleVideo(client, track, message, voiceChannel, true);
                }
                await msg.delete(msg);
                return message.channel.send({
                    embeds: [{
                ...
                if (err.message.includes("The access token expired.")) {
                    spotifyApi.refreshAccessToken().then(
                        async function (data4) {
                            ...
                            writeFile("./utils/acess.json", JSON.stringify(data4.body['access_token']), (err) => {
                                if (err) {
                                    console.error(err);
                                    return;
                                };
