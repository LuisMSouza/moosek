...
module.exports.play = async (client, message, song) => {
  ...
  const ePause = client.guilds.cache
    .get("731542666277290016")
    .emojis.cache.find((emj) => emj.name === "4210635");
  const eNext = client.guilds.cache
    .get("731542666277290016")
    .emojis.cache.find((emj) => emj.name === "4210646");
  const eBack = client.guilds.cache
    .get("731542666277290016")
    .emojis.cache.find((emj) => emj.name === "4210650");
  const eStop = client.guilds.cache
    .get("731542666277290016")
    .emojis.cache.find((emj) => emj.name === "4210632");
  ...
  try {
    ...
    const collector = playingMessage.channel.createMessageComponentCollector({
      filter,
    });
      ...
      switch (b.customId) {
        case "pause":
          if (!serverQueue) {
            await b.update({
              embeds: [],
              content: "`Não há nenhuma música sendo reproduzida`",
            });
            return;
          }
