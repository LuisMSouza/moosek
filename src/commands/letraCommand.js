...
import lyrics from "@youka/lyrics";
import { authorization, search as _search } from "play-dl";

////////////////// SOURCE CODE /////////////////////
export const name = "lyrics";
export const description = "Para pegar a letra de uma música";
...
export const usage = [process.env.PREFIX_KEY + "lyrics [nome da música]"];
export const category = "user";
export const timeout = 7000;
export const aliases = ["letra", "l"];
export async function execute(client, message, args) {
  ...
  if (!SearchStr) {
    if (serverQueue) {
      try {
        ...
        const search = await getBasicInfo(serverQueue.songs[0].url);
        await lyrics(
          `${search.videoDetails.media.song} ${search.videoDetails.media.artist}`
        ).then(async (r) => {
          await generateEmbeds(
            message,
            r,
            search.videoDetails.media.song,
    ...
    try {
      message.reply("🔎 Aguardando busca...");
      authorization();
      const input = await _search(`${SearchStr}`);
      const search = await getBasicInfo(input[0].url);
      await lyrics(
        `${
          search.videoDetails.media.song
            ? search.videoDetails.media.song
  ...
  async function generateEmbeds(message, lyrics, title, thumb, artist) {
    let embed = new MessageEmbed()
      ...
      .setThumbnail(`${thumb}`)
      .setColor("YELLOW");
    let embed2 = new MessageEmbed().setColor("YELLOW");
    let embed3 = new MessageEmbed().setColor("YELLOW");
    let embed4 = new MessageEmbed().setColor("YELLOW");
