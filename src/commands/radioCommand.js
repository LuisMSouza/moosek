...
import {
  radioStationsName,
  radioStations as _radioStations,
} from "../utils/radioStations.js";
...
export const description = "Inicia o streaming de radio";
export const usage = [
  process.env.PREFIX_KEY + "radio [numero da radio na lista]",
];
...
export async function execute(client, message, args) {
  ...
  if (radioListen)
    return sendError("A radio já está sendo executada.", message.channel);
  if (!choice) {
    ...
    collector.on("collect", async (m) => {
      m.reply({
        content: `**Você selecionou: __${
          radioStationsName[m.values[0] - 1]
        }__**`,
        ephemeral: true,
      });
      switch (m.values[0]) {
  ...
  async function initRadio(msg, bot, args, choice, choiceName) {
    try {
      ...
      const embedRadio = new _MessageEmbed()
      ...
      const row = new MessageActionRow().addComponents(button1, buttonStop);
      ...
      const colletcButt = buttonMsg.channel.createMessageComponentCollector({
        filter,
      });
      colletcButt.on("collect", async (b) => {
        if (b.customId === "stop_radio") {
          ...
          const newRow = new MessageActionRow().addComponents(button2);
      ...
      if (e.message === "Unknown stream type")
        return sendError("Radio não encontrada :(", msg.channel);
      return (
        sendError("Ocorreu um erro ao tentar executar a radio.", msg.channel) &&
        console.log(e)
      );
    }
  }
