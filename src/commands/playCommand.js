...
import { QUEUE_LIMIT } from "../utils/botUtils.js";
import { play } from "../structures/createPlayer.js";
import { handleVideo } from "../structures/strPlaylist.js";
import { handleSpotifyMusic } from "../structures/strSptfHandle.js";
import { deezerHandler } from "../structures/strDeezerHandle.js";
import { joinVoiceChannel } from "@discordjs/voice";
const guild_main = process.env.SERVER_MAIN;

/////////////////////// SOURCE CODE ///////////////////////////
export const name = "play";
export const description = "Para tocar músicas no servidor";
...
export const aliases = ["p", "play", "iniciar"];
export async function execute(client, message, args) {
  ...
  if (!voiceChannel)
    return message.reply({
      content: "Você precisa estar em um canal de voz para iniciar uma música!",
      ephemeral: true,
    });
  ...
  if (radioListen)
    return sendError("Você deve parar a radio primeiro.", message.channel);

  if (isSoundCloud) {
    try {
      if (url.includes("/sets/")) {
        SoundCloudPlaylist(url).then((res) => {
          console.log(res);
        });
      } else {
        SoundCloudTrack(url).then((res) => {
          console.log(res);
        });
      }
    } catch (e) {}
  ...
  if (isSptf) {
    ...
    await handleSpotifyMusic(client, searchString, cath, message, voiceChannel);
    if (message.options) {
  ...
  if (isPlaylist) {
    try {
      ...
      for (const video of videos) {
        await handleVideo(client, video, message, voiceChannel, true);
      }
      return message.reply({
        embeds: [
          {
    ...
    try {
      authorization();
      await search(`${searchString}`, { limit: 1 }).then(async (x) => {
        ...
        if (serverQueue) {
          if (message.guild.me.voice.channel.id !== voiceChannel.id)
            return sendError(
              "Ops :( Parece que você não está no mesmo canal que eu...",
              serverQueue.textChannel
            );
